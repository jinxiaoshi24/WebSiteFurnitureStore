    ...
    if (isset($_SESSION["damua"])){
        if (isset($_POST['btn_thanhtoan']))
        {
            ...
            if(isset($_SESSION['user']['id']))
            {
                ...
                $sql = "INSERT INTO don_hang (id_don_hang,id_khach_hang,ghi_chu,id_user) values(NULL, $last_id,'$comment','$id_user')";
            } 
            else 
            {
                $sql = "INSERT INTO don_hang (id_don_hang,id_khach_hang,ghi_chu,id_user) values(NULL, $last_id,'$comment','0')";          
            }
            ...
            echo "<script> alert('Cảm ơn bạn đã đặt hàng'); window.location='../'; </script>";
        }
        header('location: ../');
    }
